---
name: bos-onboard
description: Premier onboarding BOS : collecte profil, business et objectif, puis diagnostic initial et quick win.
version: 1.0.0
metadata:
  hermes:
...
Premier setup d'un nouvel utilisateur BOS. Déclenché automatiquement quand `workspace/BOS/Core/Profile.md` est vide ou n'existe pas.

## Objectif

En ~15 minutes, passer de zéro à un BOS entièrement peuplé :
1. `workspace/BOS/Core/Profile.md` — rempli
2. `workspace/BOS/Core/Business.md` — rempli (ou marqué pré-lancement avec les infos disponibles)
3. `workspace/BOS/Core/Goal.md` — rempli
4. `workspace/BOS/Core/Diagnosis.md` — généré (top 5 problèmes classés par impact)
5. `workspace/BOS/Core/Actions.md` — généré (actions stratégiques, priorisées)
...
4. **Générer Core/Diagnosis.md :**
   ...
   - Croiser avec `workspace/BOS/Knowledge/Common_Problems.md` pour repérer les patterns
