---
name: bos-digestion
description: Réparer les opérations, la qualité, la rétention, le recrutement et les systèmes de livraison.
version: 1.0.0
metadata:
  hermes:
...
Gérer le flux actuel quand le goulot est **opérations / qualité / rétention** — pas l'acquisition (Chase), pas le mindset fondateur (Mindset). Déclenché quand `bos-diagnosis` identifie un goulot de type Digestion en phase Scale.
...
3. **Sauvegarder** les livrables dans `workspace/BOS/Output/` et **mettre à jour** `workspace/BOS/Core/Actions.md`, `workspace/BOS/Core/Diagnosis.md`.
...
1. Si **recrutement** : sauvegarder le package complet dans **`workspace/BOS/Output/Hire_[Poste]_[date].md`** — scorecard, annonce, sourcing + outreach, entretiens + test, matrice, onboarding 30j.
2. **Mettre à jour** `workspace/BOS/Core/Actions.md` — publier, sourcer, entretiens, décision ; ou SOPs ; ou changements qualité.
3. **Mettre à jour** `workspace/BOS/Core/Diagnosis.md` — goulot Digestion ; People / Process / Quality ; prochain risque.
4. **Append** `workspace/BOS/Core/Journal.md` si décision importante.
...
|---------|--------|---------|
| `workspace/BOS/Output/Hire_[Poste]_[date].md` | Recrutement (Phase 2) | Package complet : scorecard, annonce, sourcing, entretiens, test, matrice, onboarding |
| `workspace/BOS/Core/Diagnosis.md` | Toujours | Sous-type Digestion ; séquence People / Process / Quality |
| `workspace/BOS/Core/Actions.md` | Toujours | Prochaines étapes ; tâches en **bold** |
| `workspace/BOS/Core/Journal.md` | Si utile | Embauche, décision qualité, mise en place SOP |
